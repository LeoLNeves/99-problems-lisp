...
Hint: The problem is similar to problem [P10](#p10).
...
Goldbach's conjecture says that every positive even number greater than 2 is the sum of two prime numbers. Example: 28 = 5 + 23. It is one of the most famous facts in number theory that has not been proved to be correct in the general case. It has been _numerically_ confirmed up to very large numbers (much larger than we can go with our Lisp system). Write a function to find the two prime numbers that sum up to a given even integer.
...
You can check your functions using these example trees. They are given as test cases in [p54.lisp](p54.lisp).

**[P54A](p54A.lisp) (\*) Check whether a given expression represents a binary tree**
...
**[P56](p56.lisp) (\*\*) Symmetric binary trees**
...
![](p65.gif) An alternative layout method is depicted in the illustration opposite. Find out the rules and write the corresponding Lisp function. Hint: On a given level, the horizontal distance between neighboring nodes is constant.  
...
![](p66.gif) Yet another layout strategy is shown in the illustration opposite. The method yields a very compact layout while maintaining a certain symmetry in every node. Find out the rules and write the corresponding Lisp function. Hint: Consider the horizontal distance between a node and its successor nodes. How tight can you pack together two subtrees to construct the combined binary tree?  
...
**[a)](p67a.lisp)** Write a Lisp function which generates this string representation, if the tree is given as usual (as nil or (X L R) expression). Then write a function which does this inverse; i.e. given the string representation, construct the tree in the usual form.  
...
**[a)](p68a.lisp)** Write functions preorder and inorder that construct the preorder and inorder sequence of a given binary tree, respectively. The results should be lists, e.g. (A B D E C F G) for the preorder sequence of the example in problem P67.  
  
**[b)](p68b.lisp)** Can you write the inverse of preorder from problem part a) ; i.e. given a preorder sequence, construct a corresponding tree?  
  
**[c)](p68c.lisp)** If both the preorder sequence and the inorder sequence of the nodes of a binary tree are given, then the tree is determined unambiguously. Write a function pre-in-tree that does the job.  
...
**[P70B](p70B.lisp) (\*) Check whether a given expression represents a multiway tree**
...
**[P73](p73.lisp) (\*\*) Prolog-like tree representation**
...
![](p70.gif) In Prolog we represent a multiway tree by a term t(X,F), where X denotes the root node and F denotes the forest of successor trees (a Prolog list). The example tree depicted opposite is represented by the following Lisp expression:
...
There are several ways to represent graphs in Lisp. ![](graph1.gif) One method is to represent the whole graph as one data object. According to the definition of the graph as a pair of two sets (nodes and edges), we may use the following Lisp expression to represent the example graph:

((b c d f g h k) ( (b c) (b f) (c f) (f k) (g h) ))

We call this _graph-expression form_. Note, that the lists are kept sorted, they are really _sets_, without duplicated elements. Each edge appears only once in the edge list; i.e. an edge from a node x to another node y is represented as (x y), the expression (y x) is not present. **The graph-expression form is our default representation.** In Common Lisp there are predefined functions to work with sets.
...
Write a function that calculates a numbering scheme for a given tree. What is the solution for the [larger tree](p92b.dat) pictured above?
...
In a K-regular graph all nodes have degree K; i.e. the number of edges incident to each node is K. How many (non-isomorphic!) 3-regular graphs with 6 nodes are there? See also a [table of results](p94.txt).
...
(2) Reading the data file is a tricky problem for which a solution is provided in the file [p99-readfile.lisp](p99-readfile.lisp). Use the function read-lines, which returns the words and the grid in a 2-element list.  
...
Last modified: Thu Sep 19 07:02:13 BRT 2013